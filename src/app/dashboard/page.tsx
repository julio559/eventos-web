...
// Componente principal do Dashboard que só renderiza no cliente
  ...
  const fetchData = async () => {
    try {
      ...
      const [statsRes, eventsRes, reservationsRes, establishmentRes] = await Promise.all([
        fetch('/api/establishments/stats', { headers }).catch(() => null),
        fetch('/api/events', { headers }).catch(() => null),
        fetch('/api/reservations', { headers }).catch(() => null),
        fetch('/api/establishments', { headers }).catch(() => null)
      ])
...
const ClientOnlyDashboard = dynamic(() => Promise.resolve(DashboardContent), {
  ssr: false,
  loading: () => (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600"></div>
